...
class SPyNet(nn.Module):
    ...
    def compute_flow(self, ref, supp):
        ...
        for level in range(5):
            ref.append(
                F.avg_pool2d(input=ref[-1], kernel_size=2, stride=2, count_include_pad=False)
            )
            supp.append(
                F.avg_pool2d(input=supp[-1], kernel_size=2, stride=2, count_include_pad=False)
            )
        ref = ref[::-1]
        supp = supp[::-1]
        ...
        for level in range(len(ref)):
            if level == 0:
                ...
                flow_up = (
                    F.interpolate(input=flow, scale_factor=2, mode="bilinear", align_corners=True)
                    * 2.0
                )
